...
export class AssetDetail extends LitElement {
  ...
  private async _deleteAsset() {
    ...
    try {
      await this.hass.callApi("DELETE", `property_manager/assets/${this.asset.id}`);
      this.dispatchEvent(
  ...
  render() {
    ...
    return html`
      ...
      <div class="body">
        ...
        ${asset.type
          ? html`<div style="margin-top: 8px; font-size: 14px; color: var(--pm-text-secondary)">
              Type: ${asset.type}
            </div>`
          : nothing}
                  ...
                  (s) => html`
                    <div class="schedule-item">
                      ...
                      <span class="schedule-due"
                        >${s.next_due ? `Due: ${s.next_due}` : s.frequency}</span
                      >
                    </div>
                  `
                )}
        ...
        <div class="section">
          ...
          ${asset.photos.length === 0
            ? html`<p style="font-size: 14px; color: var(--pm-text-secondary)">
                No photos yet.
              </p>`
            : nothing}
        </div>
                ...
                ${asset.maintenance_log.map(
                  (m) => html`
                    <div style="font-size: 13px; padding: 4px 0">
                      ...
                      ${m.notes ? html`<br /><em>${m.notes}</em>` : nothing}
                    </div>
                  `
                )}
              </div>
      ...
      <div class="actions">
        <button class="primary">Mark Checked</button>
        <button>Edit</button>
